import { BadgeCheck, Boxes, FileText, FlaskConical, Layers3, Package } from "lucide-react";
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
        <section className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <MetricCard label="Lotes activos" value={activeBatches.length} icon={Layers3} />
            <MetricCard label="Stock en planta" value={inventoryWithStock.length} icon={Boxes} />
            <MetricCard label="Movimientos" value={movements.length} icon={FileText} />
            <MetricCard label="Finalizados" value={completedBatches.length} icon={BadgeCheck} />
          </div>

          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Lotes en curso</h2>
                <p className="text-sm text-slate-500">Acceso rápido a la operación de planta</p>
              </div>
              ...
              <Table>
                ...
                <TableBody>
                  {loadingBatches ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="py-14 text-center text-slate-500">
                    ...
                    activeBatches.slice(0, 6).map((batch: any) => (
                      <TableRow key={batch.id}>
                        <TableCell className="font-semibold">{batch.batchNumber}</TableCell>
                        <TableCell className="text-slate-600">
                          {batch.type === "kefir_production" ? "Elaboración de Kéfir" : batch.type}
                        </TableCell>
                        <TableCell>
                          <Badge className="bg-amber-100 text-amber-700 border-amber-200">En Progreso</Badge>
                        </TableCell>
                        <TableCell className="text-slate-500">{safeFormat(batch.startDate || batch.createdAt, "dd/MM/yyyy HH:mm")}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <div className="rounded-2xl bg-sky-50 p-4">
                ...
                <p className="mt-1 text-2xl font-black text-sky-950">{inventoryWithStock.length}</p>
              </div>
          ...
          <div className="rounded-3xl bg-gradient-to-br from-emerald-950 to-slate-900 p-5 text-white shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.25em] text-emerald-300">Nódulos</p>
            ...
            <p className="mt-4 max-w-[18rem] text-sm text-emerald-100/80">
              Vista preparada para separar nódulos, reportes y control de rendimiento en submódulos propios.
            </p>
            <Button className="mt-5 w-full rounded-full bg-indigo-600 text-white hover:bg-indigo-500" asChild>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-2">
              <QuickLink href="/kefir-control/inventory" label="Inventario de Producción" />
              <QuickLink href="/kefir-control/kardex" label="Kárdex de Planta" />
            </div>
          </div>
            ...
            <div className="mt-4 space-y-3">
              {loadingMovements ? (
                ...
                movements.slice(0, 4).map((mov: any) => (
                  <div key={mov.id} className="rounded-2xl border border-slate-100 p-3">
...
function QuickLink({ href, label }: { href: string; label: string }) {
  return (
    <Button variant="outline" className="justify-start rounded-2xl border-slate-200" asChild>
